//Test
export const COUNTER_UP = "COUNTER_UP";
export const COUNTER_DOWN = "COUNTER_DOWN";
//GLOBAL
//MESSAGES
/**@constant {string} MESSAGE_LAUNCH_REQUEST*/
export const MESSAGE_LAUNCH_REQUEST = "MESSAGE_LAUNCH_REQUEST";
/**@constant {string} MESSAGE_LAUNCH_FAIL*/
export const MESSAGE_LAUNCH_FAIL = "MESSAGE_LAUNCH_FAIL";
/**@constant {string} MESSAGE_LAUNCH_SUCCESS*/
export const MESSAGE_LAUNCH_SUCCESS = "MESSAGE_LAUNCH_SUCCESS";
//AllowedIP SET
/**@constant {string} ALLOWED_IP_GET_REQUEST*/
export const ALLOWED_IP_GET_REQUEST = "ALLOWED_IP_GET_REQUEST";
/**@constant {string} ALLOWED_IP_GET_FAIL*/
export const ALLOWED_IP_GET_FAIL = "ALLOWED_IP_GET_FAIL";
/**@constant {string} ALLOWED_IP_GET_SUCCESS*/
export const ALLOWED_IP_GET_SUCCESS = "ALLOWED_IP_GET_SUCCESS";

//USER
//Auth
/**@constant {string} USER_AUTH_REQUEST*/
export const USER_AUTH_REQUEST = "USER_AUTH_REQUEST";
/**@constant {string} USER_AUTH_FAIL*/
export const USER_AUTH_FAIL = "USER_AUTH_FAIL";
/**@constant {string} USER_AUTH_ERROR*/
export const USER_AUTH_ERROR = "USER_AUTH_ERROR";
/**@constant {string} USER_AUTH_SUCCESS*/
export const USER_AUTH_SUCCESS = "USER_AUTH_SUCCESS";
//Logout
/**@constant {string} USER_LOGOUT_REQUEST*/
export const USER_LOGOUT_REQUEST = "USER_LOGOUT_REQUEST";
/**@constant {string} USER_LOGOUT_FAIL*/
export const USER_LOGOUT_FAIL = "USER_LOGOUT_FAIL";
/**@constant {string} USER_LOGOUT_ERROR*/
export const USER_LOGOUT_ERROR = "USER_LOGOUT_ERROR";
/**@constant {string} USER_LOGOUT_SUCCESS*/
export const USER_LOGOUT_SUCCESS = "USER_LOGOUT_SUCCESS";

//SET Profile
/**@constant {string} USER_PROFILE_SET_REQUEST*/
export const USER_PROFILE_SET_REQUEST = "USER_PROFILE_SET_REQUEST";
/**@constant {string} USER_PROFILE_SET_FAIL*/
export const USER_PROFILE_SET_FAIL = "USER_PROFILE_SET_FAIL";
/**@constant {string} USER_PROFILE_SET_ERROR*/
export const USER_PROFILE_SET_ERROR = "USER_PROFILE_SET_ERROR";
/**@constant {string} USER_PROFILE_SET_SUCCESS*/
export const USER_PROFILE_SET_SUCCESS = "USER_PROFILE_SET_SUCCESS";

//GET Profile
/**@constant {string} USER_PROFILE_GET_REQUEST*/
export const USER_PROFILE_GET_REQUEST = "USER_PROFILE_GET_REQUEST";
/**@constant {string} USER_PROFILE_GET_FAIL*/
export const USER_PROFILE_GET_FAIL = "USER_PROFILE_GET_FAIL";
/**@constant {string} USER_PROFILE_GET_ERROR*/
export const USER_PROFILE_GET_ERROR = "USER_PROFILE_GET_ERROR";
/**@constant {string} USER_PROFILE_GET_SUCCESS*/
export const USER_PROFILE_GET_SUCCESS = "USER_PROFILE_GET_SUCCESS";

//CheckAuth
/**@constant {string} USER_CHECK_AUTH_REQUEST*/
export const USER_CHECK_AUTH_REQUEST = "USER_CHECK_AUTH_REQUEST";
/**@constant {string} USER_CHECK_AUTH_FAIL*/
export const USER_CHECK_AUTH_FAIL = "USER_CHECK_AUTH_FAIL";
/**@constant {string} USER_CHECK_AUTH_ERROR*/
export const USER_CHECK_AUTH_ERROR = "USER_CHECK_AUTH_ERROR";
/**@constant {string} USER_CHECK_AUTH_SUCCESS*/
export const USER_CHECK_AUTH_SUCCESS = "USER_CHECK_AUTH_SUCCESS";

//List
/**@constant {string} USER_LOAD_REQUEST*/
export const USER_LOAD_REQUEST = "USER_LOAD_REQUEST";
/**@constant {string} USER_LOAD_FAIL*/
export const USER_LOAD_FAIL = "USER_LOAD_FAIL";
/**@constant {string} USER_LOAD_ERROR*/
export const USER_LOAD_ERROR = "USER_LOAD_ERROR";
/**@constant {string} USER_LOAD_SUCCESS*/
export const USER_LOAD_SUCCESS = "USER_LOAD_SUCCESS";

//Create
/** @constant {string} USER_CREATE_REQUEST*/
export const USER_CREATE_REQUEST = "USER_CREATE_REQUEST";
/** @constant {string} USER_CREATE_FAIL*/
export const USER_CREATE_FAIL = "USER_CREATE_FAIL";
/** @constant {string} USER_CREATE_ERROR*/
export const USER_CREATE_ERROR = "USER_CREATE_ERROR";
/** @constant {string} USER_CREATE_SUCCESS*/
export const USER_CREATE_SUCCESS = "USER_CREATE_SUCCESS";

//Delete
/** @constant {string} USER_DELETE_REQUEST*/
export const USER_DELETE_REQUEST = "USER_DELETE_REQUEST";
/** @constant {string} USER_DELETE_FAIL*/
export const USER_DELETE_FAIL = "USER_DELETE_FAIL";
/** @constant {string} USER_DELETE_ERROR*/
export const USER_DELETE_ERROR = "USER_DELETE_ERROR";
/** @constant {string} USER_DELETE_SUCCESS*/
export const USER_DELETE_SUCCESS = "USER_DELETE_SUCCESS";

//GET
/** @constant {string} USER_GET_REQUEST*/
export const USER_GET_REQUEST = "USER_GET_REQUEST";
/** @constant {string} USER_GET_FAIL*/
export const USER_GET_FAIL = "USER_GET_FAIL";
/** @constant {string} USER_GET_ERROR*/
export const USER_GET_ERROR = "USER_GET_ERROR";
/** @constant {string} USER_GET_SUCCESS*/
export const USER_GET_SUCCESS = "USER_GET_SUCCESS";

//UPDATE
/** @constant {string} USER_UPDATE_REQUEST*/
export const USER_UPDATE_REQUEST = "USER_UPDATE_REQUEST";
/** @constant {string} USER_UPDATE_FAIL*/
export const USER_UPDATE_FAIL = "USER_UPDATE_FAIL";
/** @constant {string} USER_UPDATE_ERROR*/
export const USER_UPDATE_ERROR = "USER_UPDATE_ERROR";
/** @constant {string} USER_UPDATE_SUCCESS*/
export const USER_UPDATE_SUCCESS = "USER_UPDATE_SUCCESS";

//CUSTOMERS
//List
/** @constant {string} CUSTOMERS_LOAD_REQUEST*/
export const CUSTOMERS_LOAD_REQUEST = "CUSTOMERS_LOAD_REQUEST";
/** @constant {string} CUSTOMERS_LOAD_FAIL*/
export const CUSTOMERS_LOAD_FAIL = "CUSTOMERS_LOAD_FAIL";
/** @constant {string} CUSTOMERS_LOAD_ERROR*/
export const CUSTOMERS_LOAD_ERROR = "CUSTOMERS_LOAD_ERROR";
/** @constant {string} CUSTOMERS_LOAD_SUCCESS*/
export const CUSTOMERS_LOAD_SUCCESS = "CUSTOMERS_LOAD_SUCCESS";

//Create
/** @constant {string} CUSTOMERS_CREATE_REQUEST*/
export const CUSTOMERS_CREATE_REQUEST = "CUSTOMERS_CREATE_REQUEST";
/** @constant {string} CUSTOMERS_CREATE_FAIL*/
export const CUSTOMERS_CREATE_FAIL = "CUSTOMERS_CREATE_FAIL";
/** @constant {string} CUSTOMERS_CREATE_ERROR*/
export const CUSTOMERS_CREATE_ERROR = "CUSTOMERS_CREATE_ERROR";
/** @constant {string} CUSTOMERS_LOAD_SUCCESS*/
export const CUSTOMERS_CREATE_SUCCESS = "CUSTOMERS_CREATE_SUCCESS";

//Delete
/** @constant {string} CUSTOMERS_DELETE_REQUEST*/
export const CUSTOMERS_DELETE_REQUEST = "CUSTOMERS_DELETE_REQUEST";
/** @constant {string} CUSTOMERS_CREATE_FAIL*/
export const CUSTOMERS_DELETE_FAIL = "CUSTOMERS_DELETE_FAIL";
/** @constant {string} CUSTOMERS_CREATE_ERROR*/
export const CUSTOMERS_DELETE_ERROR = "CUSTOMERS_DELETE_ERROR";
/** @constant {string} CUSTOMER_DELETE_SUCCESS*/
export const CUSTOMERS_DELETE_SUCCESS = "CUSTOMERS_DELETE_SUCCESS";

//Get
/** @constant {string} CUSTOMERS_GET_REQUEST*/
export const CUSTOMERS_GET_REQUEST = "CUSTOMERS_GET_REQUEST";
/** @constant {string} CUSTOMERS_GET_FAIL*/
export const CUSTOMERS_GET_FAIL = "CUSTOMERS_GET_FAIL";
/** @constant {string} CUSTOMERS_GET_ERROR*/
export const CUSTOMERS_GET_ERROR = "CUSTOMERS_GET_ERROR";
/** @constant {string} CUSTOMER_GET_SUCCESS*/
export const CUSTOMERS_GET_SUCCESS = "CUSTOMERS_GET_SUCCESS";

//Update
/** @constant {string} CUSTOMERS_UPDATE_REQUEST*/
export const CUSTOMERS_UPDATE_REQUEST = "CUSTOMERS_UPDATE_REQUEST";
/** @constant {string} CUSTOMERS_UPDATE_FAIL*/
export const CUSTOMERS_UPDATE_FAIL = "CUSTOMERS_UPDATE_FAIL";
/** @constant {string} CUSTOMERS_UPDATE_ERROR*/
export const CUSTOMERS_UPDATE_ERROR = "CUSTOMERS_UPDATE_ERROR";
/** @constant {string} CUSTOMERS_UPDATE_SUCCESS*/
export const CUSTOMERS_UPDATE_SUCCESS = "CUSTOMERS_UPDATE_SUCCESS";

//INSURERS
//List
/** @constant {string} INSURERS_LIST_REQUEST*/
export const INSURERS_LIST_REQUEST = "INSURERS_LIST_REQUEST";
/** @constant {string} INSURERS_LIST_FAIL*/
export const INSURERS_LIST_FAIL = "INSURERS_LIST_FAIL";
/** @constant {string} INSURERS_LIST_ERROR*/
export const INSURERS_LIST_ERROR = "INSURERS_LIST_ERROR";
/** @constant {string} INSURERS_LIST_SUCCESS*/
export const INSURERS_LIST_SUCCESS = "INSURERS_LIST_SUCCESS";
//Create
/** @constant {string} INSURERS_CREATE_REQUEST*/
export const INSURERS_CREATE_REQUEST = "INSURERS_CREATE_REQUEST";
/** @constant {string} INSURERS_CREATE_FAIL*/
export const INSURERS_CREATE_FAIL = "INSURERS_CREATE_FAIL";
/** @constant {string} INSURERS_CREATE_ERROR*/
export const INSURERS_CREATE_ERROR = "INSURERS_CREATE_ERROR";
/** @constant {string} INSURERS_CREATE_SUCCESS*/
export const INSURERS_CREATE_SUCCESS = "INSURERS_CREATE_SUCCESS";
//Delete
/** @constant {string} INSURERS_DELETE_REQUEST*/
export const INSURERS_DELETE_REQUEST = "INSURERS_DELETE_REQUEST";
/** @constant {string} INSURERS_DELETE_FAIL*/
export const INSURERS_DELETE_FAIL = "INSURERS_DELETE_FAIL";
/** @constant {string} INSURERS_DELETE_ERROR*/
export const INSURERS_DELETE_ERROR = "INSURERS_DELETE_ERROR";
/** @constant {string} INSURERS_DELETE_SUCCESS*/
export const INSURERS_DELETE_SUCCESS = "INSURERS_DELETE_SUCCESS";
//Get
/** @constant {string} INSURERS_GET_REQUEST*/
export const INSURERS_GET_REQUEST = "INSURERS_GET_REQUEST";
/** @constant {string} INSURERS_GET_FAIL*/
export const INSURERS_GET_FAIL = "INSURERS_GET_FAIL";
/** @constant {string} INSURERS_GET_ERROR*/
export const INSURERS_GET_ERROR = "INSURERS_GET_ERROR";
/** @constant {string} INSURERS_GET_SUCCESS*/
export const INSURERS_GET_SUCCESS = "INSURERS_GET_SUCCESS";
//UPDATE
/** @constant {string} INSURERS_UPDATE_REQUEST*/
export const INSURERS_UPDATE_REQUEST = "INSURERS_UPDATE_REQUEST";
/** @constant {string} INSURERS_UPDATE_FAIL*/
export const INSURERS_UPDATE_FAIL = "INSURERS_UPDATE_FAIL";
/** @constant {string} INSURERS_UPDATE_ERROR*/
export const INSURERS_UPDATE_ERROR = "INSURERS_UPDATE_ERROR";
/** @constant {string} INSURERS_UPDATE_SUCCESS*/
export const INSURERS_UPDATE_SUCCESS = "INSURERS_UPDATE_SUCCESS";

//SALES
//List
/**@constant {string} SALES_LIST_REQUEST*/
export const SALES_LIST_REQUEST = "SALES_LIST_REQUEST";
/**@constant {string} SALES_LIST_FAIL*/
export const SALES_LIST_FAIL = "SALES_LIST_FAIL";
/**@constant {string} SALES_LIST_ERROR*/
export const SALES_LIST_ERROR = "SALES_LIST_ERROR";
/**@constant {string} SALES_LIST_SUCCESS */
export const SALES_LIST_SUCCESS = "SALES_LIST_SUCCESS";
//Create
/**@constant {string} SALES_CREATE_REQUEST*/
export const SALES_CREATE_REQUEST = "SALES_CREATE_REQUEST";
/**@constant {string} SALES_CREATE_FAIL*/
export const SALES_CREATE_FAIL = "SALES_CREATE_FAIL";
/**@constant {string} SALES_CREATE_ERROR*/
export const SALES_CREATE_ERROR = "SALES_CREATE_ERROR";
/**@constant {string} SALES_CREATE_SUCCESS */
export const SALES_CREATE_SUCCESS = "SALES_CREATE_SUCCESS";
//Get
/**@constant {string} SALES_GET_REQUEST*/
export const SALES_GET_REQUEST = "SALES_GET_REQUEST";
/**@constant {string} SALES_GET_FAIL*/
export const SALES_GET_FAIL = "SALES_GET_FAIL";
/**@constant {string} SALES_GET_ERROR*/
export const SALES_GET_ERROR = "SALES_GET_ERROR";
/**@constant {string} SALES_GET_SUCCESS */
export const SALES_GET_SUCCESS = "SALES_GET_SUCCESS";
//Delete
/**@constant {string} SALES_DELETE_REQUEST*/
export const SALES_DELETE_REQUEST = "SALES_DELETE_REQUEST";
/**@constant {string} SALES_DELETE_FAIL*/
export const SALES_DELETE_FAIL = "SALES_DELETE_FAIL";
/**@constant {string} SALES_DELETE_ERROR*/
export const SALES_DELETE_ERROR = "SALES_DELETE_ERROR";
/**@constant {string} SALES_DELETE_SUCCESS */
export const SALES_DELETE_SUCCESS = "SALES_DELETE_SUCCESS";
//Update
/**@constant {string} SALES_UPDATE_REQUEST*/
export const SALES_UPDATE_REQUEST = "SALES_UPDATE_REQUEST";
/**@constant {string} SALES_UPDATE_FAIL*/
export const SALES_UPDATE_FAIL = "SALES_UPDATE_FAIL";
/**@constant {string} SALES_UPDATE_ERROR*/
export const SALES_UPDATE_ERROR = "SALES_UPDATE_ERROR";
/**@constant {string} SALES_UPDATE_SUCCESS */
export const SALES_UPDATE_SUCCESS = "SALES_UPDATE_SUCCESS";

//REPORT
//Sales
/**@constant {string} REPORT_LIST_REQUEST*/
export const REPORT_LIST_REQUEST = "REPORT_LIST_REQUEST";
/**@constant {string} REPORT_LIST_FAIL*/
export const REPORT_LIST_FAIL = "REPORT_LIST_FAIL";
/**@constant {string} REPORT_LIST_ERROR*/
export const REPORT_LIST_ERROR = "REPORT_LIST_ERROR";
/**@constant {string} REPORT_LIST_SUCCESS */
export const REPORT_LIST_SUCCESS = "REPORT_LIST_SUCCESS";
//Salary
/**@constant {string} REPORT_SALARY_REQUEST*/
export const REPORT_SALARY_REQUEST = "REPORT_SALARY_REQUEST";
/**@constant {string} REPORT_SALARY_FAIL*/
export const REPORT_SALARY_FAIL = "REPORT_SALARY_FAIL";
/**@constant {string} REPORT_SALARY_ERROR*/
export const REPORT_SALARY_ERROR = "REPORT_SALARY_ERROR";
/**@constant {string} REPORT_SALARY_SUCCESS */
export const REPORT_SALARY_SUCCESS = "REPORT_SALARY_SUCCESS";
//Profit
/**@constant {string} REPORT_PROFIT_REQUEST*/
export const REPORT_PROFIT_REQUEST = "REPORT_PROFIT_REQUEST";
/**@constant {string} REPORT_PROFIT_FAIL*/
export const REPORT_PROFIT_FAIL = "REPORT_PROFIT_FAIL";
/**@constant {string} REPORT_PROFIT_ERROR*/
export const REPORT_PROFIT_ERROR = "REPORT_PROFIT_ERROR";
/**@constant {string} REPORT_PROFIT_SUCCESS */
export const REPORT_PROFIT_SUCCESS = "REPORT_PROFIT_SUCCESS";

//DASHBOARD
//List
/**@constant {string} DASHBOARD_GET_REQUEST*/
export const DASHBOARD_GET_REQUEST = "DASHBOARD_GET_REQUEST";
/**@constant {string} DASHBOARD_GET_FAIL*/
export const DASHBOARD_GET_FAIL = "DASHBOARD_GET_FAIL";
/**@constant {string} DASHBOARD_GET_ERROR*/
export const DASHBOARD_GET_ERROR = "DASHBOARD_GET_ERROR";
/**@constant {string} DASHBOARD_GET_SUCCESS */
export const DASHBOARD_GET_SUCCESS = "DASHBOARD_GET_SUCCESS";

//Personal Performance
/**@constant {string} DASHBOARD_PERSONAL_PERFORMANCE_REQUEST*/
export const DASHBOARD_PERSONAL_PERFORMANCE_REQUEST =
  "DASHBOARD_PERSONAL_PERFORMANCE_REQUEST";
/**@constant {string} DASHBOARD_PERSONAL_PERFORMANCE_FAIL*/
export const DASHBOARD_PERSONAL_PERFORMANCE_FAIL =
  "DASHBOARD_PERSONAL_PERFORMANCE_FAIL";
/**@constant {string} DASHBOARD_PERSONAL_PERFORMANCE_ERROR*/
export const DASHBOARD_PERSONAL_PERFORMANCE_ERROR =
  "DASHBOARD_PERSONAL_PERFORMANCE_ERROR";
/**@constant {string} DASHBOARD_PERSONAL_PERFORMANCE_SUCCESS */
export const DASHBOARD_PERSONAL_PERFORMANCE_SUCCESS =
  "DASHBOARD_PERSONAL_PERFORMANCE_SUCCESS";

//FILTER STATES
export const FILTER_SET_REQUEST = "FILTER_SET_REQUEST";
export const FILTER_SET_FAIL = "FILTER_SET_FAIL";
export const FILTER_SET_SUCCESS = "FILTER_SET_SUCCESS";
